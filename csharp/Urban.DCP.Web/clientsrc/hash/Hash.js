 ...
 *     Hash.go('abc123');
 * 
 * Updated by Aaron Ogle (aogle@azavea.com) on 8/11/2009 to:
 *   - Make compatible with default settings of JSLint (www.jslint.com)
 *   - Fix bug where Firefox automatically unescapes location.hash
 *     but no other browsers do. Always get the hash by parsing 
 *     location.href and never use location.hash.
 */
...
getHash = function () {
    ...
    // because the hash variable (escaped) will not match location.hash (unescaped).
    // The only consistent option is to rely completely on location.href.
